...
function renderCategoryButtons() {
    ...
    let buttonsHtml = '<button id="btn-cat-Semua" onclick="filterMenu(\'Semua\', this)" class="cat-btn bg-amber-500 text-slate-900 px-6 py-2.5 rounded-xl text-sm font-bold whitespace-nowrap smooth-transition shadow-md">Semua</button>';

    sortedCategories.forEach(function(cat) {
        const safeId = cat.replace(/\s+/g, '_');
        buttonsHtml += '<button id="btn-cat-' + safeId + '" onclick="filterMenu(\'' + cat.replace(/'/g, "\\'") + '\', this)" class="cat-btn bg-slate-800 text-slate-300 hover:bg-slate-700 px-6 py-2.5 rounded-xl text-sm font-bold whitespace-nowrap smooth-transition">' + cat + '</button>';
    });
...
function renderSubCategoryChips() {
    ...
    let chipsHtml = '<span class="text-[9px] font-black text-slate-500 uppercase tracking-widest mr-2">Sub-Menu:</span>' +
        '<button onclick="filterSubMenu(\'Semua\', this)" id="btn-sub-Semua" class="sub-chip bg-amber-500 text-slate-950 px-4 py-1.5 rounded-xl text-xs font-black whitespace-nowrap transition-all">SEMUA</button>';

    Array.from(uniqueSubs).sort().forEach(function(sub) {
        const safeSubId = sub.replace(/\s+/g, '_');
        chipsHtml += '<button onclick="filterSubMenu(\'' + sub.replace(/'/g, "\\'") + '\', this)" id="btn-sub-' + safeSubId + '" class="sub-chip bg-slate-800 text-slate-400 px-4 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all hover:text-white">' + sub.toUpperCase() + '</button>';
    });
...
function filterSubMenu(subCat, btnElement) {
    ...
    document.querySelectorAll('.sub-chip').forEach(function(b) {
...
function applyFilters(searchStr) {
    ...
    if (currentCategory !== 'Semua') { 
        filteredData = filteredData.filter(function(m) { return (m.category || "").trim() === currentCategory; }); 
    }

    if (currentSubCategory !== 'Semua') {
        filteredData = filteredData.filter(function(m) { return (m.subCategory || "").trim() === currentSubCategory; });
    }
    
    if (keyword !== "") {
        filteredData = filteredData.filter(function(m) { 
...
function renderMenuHTML(items) {
    ...
    container.innerHTML = items.map(function(item) {
        ...
        return '<div onclick="addToCart(\'' + safeId + '\', \'' + safeName + '\', ' + safePrice + ', \'' + safeRoute + '\')" class="menu-card bg-slate-800 rounded-2xl border border-slate-700 flex flex-col overflow-hidden cursor-pointer hover:border-amber-500 relative transition-all duration-300 h-full active:scale-95 text-slate-100">' +
...
function applyVoucherSelect(code) {
    if (code === "NONE") {
        ...
        const voucher = voucherData.find(function(v) { return v.code === code; });
        const subtotal = cart.reduce(function(sum, item) { return sum + item.subtotal; }, 0);
...
function addToCart(id, name, price, route) {
    ...
    const exist = cart.find(function(i) { return i.menuId === id; });
    if(exist) { 
        exist.qty++; 
...
function renderCart() {
    ...
    document.getElementById('cart-count').innerText = cart.reduce(function(sum, item) { return sum + item.qty; }, 0);
    ...
    if (mobCount) mobCount.innerText = cart.reduce(function(sum, item) { return sum + item.qty; }, 0);
    ...
    container.innerHTML = cart.map(function(item, idx) {
        return '<div class="bg-slate-800 p-3.5 rounded-2xl border border-slate-700 shadow-sm relative animate-slide-up text-slate-100">' +
